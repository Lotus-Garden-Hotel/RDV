// ═══════════════════════════════════════════════════════════
// H.O.M.T — Firebase Hybrid Bridge v2.1
...
// Expose ke window agar startPolling di HTML bisa cek
Object.defineProperty(window, '_firebaseReady', {
  get: () => _firebaseReady,
  set: v => { _firebaseReady = v; },
  configurable: true,
});
...
async function _syncToFirestore(action, payload, result, db) {
  ...
  // Timeout guard: Firestore write tidak boleh hang > 8s
  const _writeTimeout = new Promise((_, reject) =>
    setTimeout(() => reject(new Error('Firestore write timeout 8s')), 8000)
  );
